...
import { format, shouldSend } from './report.mjs'
...
test('failure-only: green sends nothing, red always sends', () => {
  assert.equal(shouldSend(true, undefined), false)
  assert.equal(shouldSend(false, undefined), true)
  assert.equal(shouldSend(true, 'true'), true, 'REPORT_ALWAYS overrides')
})
