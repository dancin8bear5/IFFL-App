...
4–1. Built from the generated payload against the analytics agent's
reference render (`IFFL_PR_CARDS__HANDOFF.md`).

**THE CONTENT IS NEVER BUNDLED, AND THE PAYLOAD IS NOT IN THIS REPO.**
`powerRankings/{edition}` holds only edition, date and `released[]`; each
section is its own doc under `/drops/{intro,12-9,8-5,4-1}` and the client
fetches only what is released. An unreleased write-up is never sent to a
browser. `web/src/data/powerRankings2026.json` is **gitignored** — it is
generated in the analytics agent and pushed straight to Firestore by
`web/scripts/publish-power-rankings.mjs` (gcloud access token + REST, same
pattern as `import-history.mjs`; `--dry-run` needs no auth).

**A dev-only import was tried and rejected.** `import.meta.env.DEV` does get
eliminated from the output, but Rollup still RESOLVES the import, so the
payload would have to stay committed to a public repo for the build to
pass. There is deliberately no import of the content anywhere in the view.
The page therefore needs Firestore to render — `npm run dev` works because
`web/.env` is real; `?preview=1` shows the empty state.

**The check that proves the embargo:** grep the built `dist/` for a phrase
from an unreleased card (`GIBBS AND ACHANE`, `THIS IS MY MASTERPIECE`) and
expect zero hits. Do NOT check that a chunk is absent — a stray import can
inline strings elsewhere. Hits on `Kyler Murray`, `Meta Knights` or
...
Cantone #5 above two teams that outscore him. The score column reads out of
order in exactly two rows. That is correct; do not "fix" it.

**Do not normalise whitespace in the voice bodies.** Blank line is a new
paragraph, single newline is a `<br>`. Abad writes one thought per line and
that shape is the joke — his card renders 44 breaks.

**Do not hand-edit page copy.** Everything is generated by
`power_rankings.py` in `~/claude-agents/agents/iffl-analytics-agent`; a
hand-edit is lost on the next run. Copy changes go back to Jared.

Two departures from the handoff, both deliberate: **system fonts** rather
than Barlow/Barlow Condensed/IBM Plex Mono (the app has no web fonts, and
three families would make one page typographically foreign — the condensed
and mono *roles* survive as tracked-out caps and `tabular-nums`), and **dark
only** (the spec's dark palette IS BeltTheme renamed; its light half has
nowhere to go in an app with no light mode). Also dropped the `.sec.locked`
middot: §7 says locked sections have no visual difference, and an
unexplained dot leaks generator state to readers who can't read it.

**Deep links** are `#power-rankings/2026-preseason/<slug>` (the edition
segment is optional). Two fixes were needed: `parseRoute` returned only the
first segment and threw the team away — it exposes `params` now, with
`param` unchanged so Rosters is untouched — and the view is lazy, so
TabLayout's URL writer had already normalised the hash to the bare slug
